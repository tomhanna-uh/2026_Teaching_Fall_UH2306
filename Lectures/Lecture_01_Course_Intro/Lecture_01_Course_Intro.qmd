...
- Includes: syllabus/intro quizzes, textbook access (CTAP / We the People + Governing Texas), **Top Hat registration**, Inquizitives setup
- Instructions are in Module 0 on Canvas

Register Top Hat **through the Canvas course link** (not by searching the web first).
...
- Negotiating grades, excuses, or special exceptions to the syllabus
...
I cannot fix platform, billing, or device problems for 700+ students. Use the specialized support channels.
...
- With ~750 students across sections, email need is minimal
- Class cancellations: Canvas Announcements and official email
- Other announcements: Canvas (during class time when possible)
- **Do not email** excuses, grade questions, makeup or extension requests — discuss in person at office hours if needed
- Emails not from `@cougarnet.uh.edu` or `@uh.edu` are ignored (privacy)
...
- Late Inquizitives: accepted with automatic daily penalty
- Missed midterm: final exam percentage applies automatically — no petition needed
- Missed Top Hat days: no makeup
- Policy is already generous — **no special exceptions**

Asking for special treatment is asking for an ethical problem. It can affect your professionalism score.
...
- You must get Top Hat working on your device (Top Hat support if needed)
...
- Use flashcards a few minutes, a few times a week
...
2. Register textbook access (CTAP or as directed)
